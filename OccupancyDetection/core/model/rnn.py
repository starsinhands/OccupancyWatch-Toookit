...
class LSTM(NormalModel):
    ...
    """
    def __init__(self, train, test, epoch=10):
    ...
    def run(self):
        ...
        num_classes = self.train.occupancy.shape[1]  # 假设occupancy是一个one-hot编码的矩阵

        model = Sequential()
        model.add(LSTM(batch_input_shape=(self.batch_size, 1, self.train.data.shape[1]),
                       units=self.cell,
                       return_sequences=False,  # Set to False for classification
                       stateful=True))

        # Add a Dense layer for classification with num_classes output units
        model.add(Dense(num_classes, activation='softmax'))  # Output a probability distribution for each class
        ...
        for _ in range(self.hm_epochs):
            ...
            for i in range(int(self.train.data.shape[0] / self.batch_size)):
                ...
                epoch_y = self.train.occupancy[i * self.batch_size:
                                               (i + 1) * self.batch_size].reshape(self.batch_size, num_classes)
                # Train the model on the current batch and get the loss
                loss = model.train_on_batch(epoch_x, epoch_y)
                epoch_loss += sum(loss)/len(loss)  # Accumulate the batch loss

            # Print the loss for the current epoch
            print(f'Epoch {_:02d}, Loss: {epoch_loss}')
        # ... (rest of the training loop remains the same)

        final = zeros((self.test.data.shape[0], num_classes), dtype=float)  # Adjusted shape for classification
        ...
        return final.argmax(axis=1)  # Return the index of the class with the highest probability
    #
    # def run(self):
    #
    #     from keras.models import Sequential
